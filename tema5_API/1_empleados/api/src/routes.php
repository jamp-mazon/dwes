    ...
    switch ($requestMethod) {
        ...
        case 'PUT':
            if ($userId!==null && $userId!=="") {
                ...
                if ($mibd->actualizar_empleado($userId,$datos)) {
                    $respuesta=["mensaje"=>"Usuario con ID: $userId actualizado con exito"];
                    http_response_code(200);
                    echo json_encode($respuesta);
                    exit();
                }
                else{
                    $respuesta=["error"=>"Usuario con ID:$userId no se ha podido actualizar..."];
                    http_response_code(500);
                    echo json_encode($respuesta);
                    exit();                    
                }
